package com.lihebin.sso.config;
...
//  }
    ...
    //申明一个切点 里面是 execution表达式
  @Pointcut("execution(public * com.lihebin.sso.service.*.*(..))")
  ...
  public void webAfterReturning(JoinPoint joinPoint, Object returnObject){
    log.info("[ID{}][{}] web response>>:[{}]", Thread.currentThread().getId(), joinPoint.getSignature().getName(), JSON.toJSONString(returnObject));
  }
